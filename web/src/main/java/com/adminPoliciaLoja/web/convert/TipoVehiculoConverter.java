...
public class TipoVehiculoConverter implements Converter<Tipovehiculo> {
	 
	public Tipovehiculo getAsObject(FacesContext context, UIComponent component, String value) {
		boolean isNumero=StringUtils.isNumeric(value);
		if(value != null && value.trim().length() > 0 && isNumero) {
            try {
            	
                return DaoFactory.getInstance().getTipoVehiculoDao().findOne(Integer.valueOf(value));
	...
	public String getAsString(FacesContext context, UIComponent component, Tipovehiculo object) {
